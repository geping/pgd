		...
		jQuery('#t_id').change(function(){
			...
			<div id="primary_right">
				<div class="inner">
					...
					<form method="POST" action="/moduleEdit">
					 ...
					 <fieldset>
						...
						<p>
							...
							<select name="t_id" id="t_id" class="dropdown">
								<option value="0">请选择该模块绑定的模板</option>
								<?php
									foreach($body['template'] as $key => $value) {
								?>
								<option value="<?php echo $value['id']; ?>" <?php if ($value['id'] == $body['list']['t_id']) {echo 'selected="selected"';}?>><?php echo $value['title']; ?></option>
								<?php
									}
								?>
							</select>
						...
						<p style="float:left;">
							...
							<textarea name="description" id="description" cols="60" rows="8"><?php echo $body['list']['description']; ?></textarea>
						</p>
						
						<div class="clearboth"></div>
						<input type="hidden" name="id" id="id" value="<?php echo $body['list']['id']; ?>" />
